...
put("Version of the `edx` dataset with split rows 
for movies belonging to multiple genres: edx.sgr")
put(summary(edx.sgr))
...
file_name_tmp <- "7.edx.UMG-effect.RData"
file_path_tmp <- file.path(data.models.path, file_name_tmp)
...
if (file.exists(file_path_tmp)) {
  ...
  edx.UMG_effect <- train_user_movie_genre_effect()
...
put(str(edx.UMG_effect))
###### User+Movie+Genre Effect data integrity test------------------------------------
UMGE.tst <- edx.UMG_effect |>
  mutate(tst.col = g) |>
  select(movieId, tst.col)

cv.UMGE.test.left_join.Nas <- UMGE.tst |>
  data.consistency.test.cv(by.userId = FALSE)

put_log("Below are the User+Movie+Genre Effect consistency test results")
put(cv.UMGE.test.left_join.Nas)
#      user.NAs movie.NAs
# [1,]       NA         0
# [2,]       NA         0
# [3,]       NA         0
...
stopifnot(colSums(cv.UMGE.test.left_join.Nas)["movie.NAs"] == 0)
###### Plot a histogram of the Movie Genre Effect distribution -----------------
par(cex = 0.7)
hist(edx.UMG_effect$g, 30, xlab = TeX(r'[$\hat{g}_{i,j}$]'),
...
cv.UMG_effect.RMSE <- calc_user_movie_genre_effect_RMSE.cv(edx.UMG_effect)
cv.UMG_effect.RMSE
#> [1] 0.859473
...
if (file.exists(file_path_tmp)) {
  ...
  rglr.UMG_effect <- train_user_movie_genre_effect(UMGE.rglr.best_lambda)
...
###### Regularized User+Movie+Genre Effect data integrity test------------------
UME.tst <- rglr.UM_effect |>
  mutate(tst.col = b) |>
  select(movieId, tst.col)
...
stopifnot(colSums(rglr.UME.test.left_join.Nas)["movie.NAs"] == 0)
#### Calculate RMSE for Regularized User+Movie+Genre Model ---------------------
...
if (file.exists(file_path_tmp)) {
  ...
  rglr.UMGY_effect <- train_UMGY_effect.cv(UMGYE.rglr.best_lambda)
