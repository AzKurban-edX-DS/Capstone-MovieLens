...
\begin{noteblock}
The full source code of the functions described below are are available
in the
...
\textbf{param\_values:}; \textbf{fn\_tune.test.param\_value:};
\textbf{break.if\_min = TRUE:}; \textbf{steps.beyond\_min = 2:}:.
...
\item
  \textbf{fn\_tune.test.param\_value:} A function that calculates the
  value of the \texttt{RMSE} for a given parameter value.;
\item
...
the \texttt{param\_values} parameter. During execution, the function
uses a helper function specified by the
...
\begin{noteblock}
Note that the function assumes that the dependence of the mean square
error on the input parameter is a monotonically decreasing function
until a minimum is reached and monotonically increasing thereafter. That
is, it is assumed to have a unique minimum in the domain of definition.
...
\subparagraph{`` Function}\label{function-4}
...
\includegraphics{capstone-movielens-report.draft4_files/figure-latex/unnamed-chunk-86-1.pdf}
