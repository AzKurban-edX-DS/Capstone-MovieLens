...
\tightlist
\item
  \textbf{loop\_starter:} A numeric vector of the form
  \texttt{c(start,\ end,\ dvs)}, where \texttt{start} and \texttt{end}
  are the endpoints of the interval on which the parameter value that
  minimizes \texttt{RMSE} is sought. \texttt{dvs} is a divisor for
  splitting the interval to transform it into a sequence of values among
  which the value that minimizes \texttt{RMSE} is sought. For this
  purpose, the sequence step is calculated as follows: \[
  seq_step = \frac{end - start}{dvs}
  \]
\end{itemize}
...
\tightlist
\item
  \textbf{tune\_dir\_path:} ;
\item
  \textbf{cache\_file\_base\_name:} ;
\item
  \textbf{fn\_tune.test.param\_value:} A helper function that calculates
  the value of the \texttt{RMSE} for a given parameter value.;
\item
  \textbf{max.identical.min\_RMSE.count:} ;
\item
  \textbf{is.cv:} ;
\item
