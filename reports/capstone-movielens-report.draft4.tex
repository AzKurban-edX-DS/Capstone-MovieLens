...
Function}{model.tune.param\_range Function}}\label{func.model.tune.param_range}
...
max.identical.min\_RMSE.count = 4

\hfill\break
If more than one identical minimum \texttt{RMSE} value is calculated
during execution, the number of identical minimums is limited by the
value of this parameter. When it is reached, the algorithm considers the
task execution to be complete.

endpoint.min\_diff = 0

\hfill\break
Defines the sensitivity threshold for determining the neighborhood
boundaries of the minimum \texttt{RMSE} value (for details, see the
\texttt{Details} section below).

break.if\_min = TRUE

\hfill\break
This is a parameter that is required for the
\href{https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R\#L116}{tune.model\_param}
function that is called internally during execution (see the description
of the \texttt{tune.model\_param} function
\hyperref[func.tune.model_param]{above}).
...
::: Details \{\#func.model.tune.param\_range.details \}
...
(ref:foo) During execution, the function uses a helper function
specified by the \texttt{fn\_tune.test.param\_value} parameter, which
calculates the \texttt{RMSE} value for the given parameter from the list
determined by the inner \texttt{param\_values} sequence.

Value

\hfill\break
See the \texttt{Details}section
\hyperref[func.model.tune.param_range.details]{above}. See the
\texttt{Details}section \href{ref:foo}{above}. See the
\texttt{func.model.tune.param\_range} Function description
\hyperref[func.model.tune.param_range]{above}. pdf\_document2
