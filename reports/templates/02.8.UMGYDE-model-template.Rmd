...
\begin{equation}
Y_{i,j} = \mu + \alpha_i + \beta_j + g_{i,j} + \gamma(r_{i,j}) + f(d_{i,j}) \varepsilon_{i,j}
(\#eq:UMGYDE-model)
\end{equation}

with $f$ a smooth function of $d_{i,j}$
...
$$
\hat{f} = f(d_{i,j}) + \varepsilon_{i,j}
$$

takes the following form: 

\begin{equation}
\hat{f(d_{i,j})} = Y_{i,j} - (\mu + \alpha_i + \beta_j + g_{i,j} + \gamma(r_{i,j}))
...
#### [calc_date_general_effect](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L2) Function {#func.calc_date_general_effect}
\

This is a helper function that calculates the _date general effect_ of the user ratings and is intended to be used in other functions and scripts (for instance in [train_UMGYD_effect](#func.train_UMGYD_effect)) described below.
...
::: {.noteblock data-latex=""}
The complete version of the source code of the [calc_date_general_effect](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L2) function is available in the [UMGYDE Model Support Functions](#L1) section of the [UMGYD-effect.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L2) script on _GitHub_.
:::
...
#### [calc_date_general_effect.cv](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L30) Function {#func.calc_date_general_effect.cv}
\

This is a helper function that calculates the _date general effect_ of the user ratings using the _5-Fold Cross Validation_ method and is intended to be used in other functions and scripts (for instance in [train_UMGYD_effect.cv](#func.train_UMGYD_effect.cv)) described below.

##### Souurce Code
\

::: {.noteblock data-latex=""}
The complete version of the source code of the [calc_date_general_effect.cv](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L30) function is available in the [UMGYDE Model Support Functions](#L1) section of the [UMGYD-effect.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L30) script on _GitHub_.
:::
...
```{r, eval=FALSE}
  cv.UMGYD_effect <- train_UMGYD_effect.cv()
```
```{r}
str(cv.UMGYD_effect)
summary(cv.UMGYD_effect)
```
...
```{r, eval=FALSE}
cv.UMGYD_effect.RMSE <- calc_UMGYD_effect_RMSE.cv(cv.UMGYD_effect)

RMSEs.ResultTibble.UMGYDE <- RMSEs.ResultTibble.rglr.UMGYE |> 
  RMSEs.AddRow("UMGYDE Model", 
...
\begin{equation}
\sum_{i,j} \left(y_{u,i} - \mu - \alpha_i - \beta_j - g_{i,j} - \gamma(r_{i,j}) - f(d_{i,j})\right)^2 + \lambda \sum_{i,j} f(d_{i,j})^2
(\#eq:DE-penalty)
\end{equation}
...
\begin{equation}
\hat{\gamma}(r, \lambda) = \frac{1}{\lambda + n_d} \sum_{i=1}^{n_{d}} \left(Y_{i,j} - \mu - \alpha_i - \beta_j - g_{i,j} - \gamma(r)\right)
(\#eq:DE-regularized)
\end{equation}
...
`lambda` is `NA`")
  ...
  train_UMGYD_effect.cv(lambda) |>
    calc_UMGYD_effect_RMSE.cv()
}
```
...
```{r, eval=FALSE}
  rglr.UMGYD_effect <- edx |> train_UMGYD_effect(UMGYDE.rglr.best_lambda)
...
```{r, eval=FALSE}
  rglr.UMGYD_effect.RMSE <- calc_UMGYD_effect_RMSE.cv(rglr.UMGYD_effect)
```

Finally, we add the definitive result for the current model to our _Result Table_:
```{r, eval=FALSE}
RMSEs.ResultTibble.rglr.UMGYDE <- RMSEs.ResultTibble.UMGYDE |> 
  RMSEs.AddRow("Regularized UMGYDE Model", 
               rglr.UMGYD_effect.RMSE,
               comment = "Computed for `lambda` = %1" |>
                 msg.glue(UMGYDE.rglr.best_lambda))

```
```{r}
RMSE_kable(RMSEs.ResultTibble.rglr.UMGYDE)
```
