...
::: {.noteblock data-latex=""}
The full source code of the functions described below are are available in the [Model Tuning](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L74) section of the [common-helper.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R) script on _GitHub_.
:::

##### [tune.model_param](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L116) Function
\
...
**steps.beyond_min = 2:**:.
  ...
  - **fn_tune.test.param_value:** A function that calculates the value of the `RMSE` for a given parameter value.; 
...
::: {.noteblock data-latex=""}
Note that the function assumes that the dependence of the mean square error on the input parameter is a monotonically decreasing function until a minimum is reached and monotonically increasing thereafter. That is, it is assumed to have a unique minimum in the domain of definition.
:::
