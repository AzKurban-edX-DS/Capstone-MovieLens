...
Below, we provide the most important code of the training our model using the `5-Fold Cross Validation` method:
```{r eval=FALSE}
...
::: {.noteblock data-latex=""}
The functions described below are defined in the [Model Tuning]() section of the [common-helper.functions.R]() script.
:::

##### `tune.model_param` Function
\
...
This function calculates the _RMSE_ for the given $\lambda$ parameter value:
```{r eval=FALSE}
