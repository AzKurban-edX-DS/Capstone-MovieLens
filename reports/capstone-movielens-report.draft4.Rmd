...
##### [model.tune.param_range](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L198) Function
\
...
To improve performance, the algorithm caches intermediate results in the file system. This parameter specifies the path to the directory where the cache files are saved.
...
This is a helper function name that is passed to the same-named parameter of the [tune.model_param] function that is called internally during the execution (see the [description of the `tune.model_param` function](#func.tune.model_param) above).  
...
\
  ...
  - **break.if_min = TRUE:** A Boolean parameter that determines whether the function should terminate after completing the number of steps 

#######  
\

specified by the parameter `steps.beyond_min`, after the minimum value of the `RMSE` has been found;
