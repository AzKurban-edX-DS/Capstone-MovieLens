...
We call this function internally from the functions that compute the _penalized estimates_  as described in section [Section 24.1 *Case study: recommendation systems / Penalized Least Squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-1].
...
#### [get_fine_tune.param.endpoints.idx](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#LX) Function {#func.get_fine_tune.param.endpoints.idx}
\
...
The source code of the [get_fine_tune.param.endpoints.idx](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#LX) function is shown below:
```{r eval=FALSE}
```

::: {.noteblock data-latex=""}
The source code of the [get_fine_tune.param.endpoints.idx](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#LX) function is also available in the [Regularization](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L69) section of the [common-helper.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#LX) script on _GitHub_.
:::
:::
...
Implements an algorithm to find a value of the *regularization parameter* $\lambda$ that corresponds to the minimum value of the *Root Mean Squared Error (RMSE)* for the model under test (we call this the *best value of the regularization parameter* or *best value of $\lambda$*) from the sorted *parameter vector* passed in the [param_values](#func.tune.model_param.args.param_values) argument.
...
The algorithm iterates through the $\lambda$ parameter values from the list passed in the [param_values](#func.tune.model_param.args.param_values) argument, computing the model's *RMSE* for each of them, using the helper function specified by the [fn_tune.test.param_value](#func.tune.model_param.args.fn_tune.test.param_value) argument.
...
::: {.noteblock data-latex=""}
The algorithm assumes that the dependence of *RMSE* on the parameter $\lambda$ is a monotonically decreasing function until a minimum is reached and monotonically increasing thereafter. That is, it is assumed that the function can have only one minimum on a given interval.
:::
...
This function returns a list of two objects:
  ...
  - A vector containing a *pair of values*: the found *best value of the regularization parameter* $\lambda$ for which the *RMSE* value is minimal, along with the minimal *RMSE* value itself;
```{r eval=FALSE}
  param_values.best_result <- c(param.best_value, 
                                best_RMSE)
```
...
Implements an algorithm for fine-tuning the model under test by finding the most precise value possible of the *regularization parameter* $\lambda$ that corresponds to the minimum value of the *Root Mean Squared Error (RMSE)* (we call this the *best value of the regularization parameter* or *best value of $\lambda$*) over a given interval determined by the start and end values of the interval passed in the [loop_starter] argument.
...
##### **Details** 
\
During execution, the algorithm organizes a cycle consisting of the following steps:

  1. Calculates the divisor to calculate the step size for partitioning the interval into a sequence of $\lambda$ values as described in the [loop_starter](#func.model.tune.param_range.args.loop_starter) argument description above using the following [line of code](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L240):
```{r eval=FALSE}
    seq_increment <- (seq_end - seq_start)/interval_divisor 
```

  2. Checks if the *step size* of the generated sequence is less than the minimum allowed value (in the current implementation, we use `1e-13`), and if `TRUE`, breaks out of the loop. Otherwise, the execution continues to the next step:
```{r eval=FALSE}
...
:::  
  ...
  3. If the *step size* is no less than the minimum allowed value (`seq_increment >= 1e-13` in our case) creates a vector containing a sequence of $\lambda$ values generated as described in the [loop_starter](#func.model.tune.param_range.args.loop_starter) argument description above:
```{r eval=FALSE}
    test_param_vals <- seq(seq_start, seq_end, seq_increment)
```

  4. Calls the [tune.model_param](#func.tune.model_param) helper function, passing the vector created in the previous step to the [param_values](#func.tune.model_param.args.param_values) argument (along with the values of the other arguments of the function `tune.model_param`).

returns a range of input parameter values associated with the set of corresponding _Root Mean Squared Errors_ that is also guaranteed to include their minimum value, as described in the [Value](#func.tune.model_param.value) subsection of the [tune.model_param](#func.tune.model_param) function description.
...
Trains the *(Regularized) User+Movie (UM) Effect Model* using the training dataset passed in the [train_set](#func.train_user_movie_effect.args.train_set) parameter.
...
Trains the *(Regularized) User+Movie (UM) Effect Model* with use of *K-Fold Cross-Validation* as described in [Section 30.6 *Cross validation / K-fold cross validation*](https://rafalab.dfci.harvard.edu/dsbook-part-2/ml/resampling-methods.html#k-fold-cross-validation) of the _Course Textbook (New Edition)_[@IDS2_30-6].
