...
::: { #func.mse.args.r .sidebar }
  - **r:** Numeric vector representing a set of *residuals*. 
...
::: {.noteblock data-latex=""}
In the sample code snippet above, the `mean_reg` function is called internally from the [train_user_movie_effect](#func.train_user_movie_effect) function described below in section *Models Training: Support Functions /* [UME Model: Utility Functions] of *this Appendix*.
:::
:::
...
::: { #func.mean_reg.args.lambda .sidebar }
  - **lambda:** (0 by default) _Regularization parameter_ $\lambda$. 

:::
  - **na.rm:** (`TRUE` by default) Logical value indicating whether `NA` values should be stripped before the computation proceeds.
...
::: {.noteblock data-latex=""}
This is a helper function intended for use in other helper functions that compute *penalized estimates* during the *regularization* process of the models being trained[@IDS2_24-2]. All these functions are described below in section [Models Training: Support Functions] of *this Appendix*.
:::
...
In *This Project*, this function is used internally in the [model.tune.param_range](#func.model.tune.param_range) function described below in this section.
:::
...
Implements an algorithm for selecting the best value of the *regularization parameter* $\lambda$ from the sorted *parameter vector* passed in the [param_values](#func.tune.model_param.args.param_values) argument, using a simple approach of selecting the *penalty term* as described in [Section 24.3 *Selecting the penalty term*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#sec-selecting-penalty-term)*[@IDS2_24-3].
...
The algorithm is based on a simple approach of selecting the *penalty term* as described in [Section 24.3 *Selecting the penalty term*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#sec-selecting-penalty-term)*[@IDS2_24-3].
...
- **normalize = FALSE:** If `TRUE`, deviations from the minimum $y$ value are used to plot, rather than the $y$ values themselves. Otherwise, the $y$ values are used.
...
The Mean Squared Error (MSE) values obtained from applying the *K-Fold Cross-Validation* method to the *Overall Average Rating (OMR) Model*, where the $K$ value is determined by the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
...
::: { #func.train_user_movie_effect.args.lambda .sidebar }
  - **lambda:** (0 by default) Regularization parameter $\lambda$. 
...
::: {.noteblock data-latex=""}
*Regularization* is committed through the *penalized regression* by computing the *penalized mean* performed by the internally called [mean_reg](#func.mean_reg) function described above in the *Common Helper Functions /* [Regularization Functions] section of *this Appendix*.
:::
:::
...
Data frame representing the *UM Effect Model* trained (and also *regularized* if the [lambda](#func.train_user_movie_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
The *MSE* value calculated for the *UM Effect Model* with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
The *RMSE* value calculated for the *UM Effect Model* with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
::: {.noteblock data-latex=""}
In the code snippet above this function is passed in the [fn_tune.test.param_value](#func.tune.model_param.args.fn_tune.test.param_value) argument to the [tune.model_param](#func.tune.model_param) function described above in the section *Common Helper Functions /* [Regularization Functions] of *this Appendix*.
...
::: {.noteblock data-latex=""}
This function is also intended to be passed in the `fn_tune.test.param_value` argument to the [tune.model_param](#func.tune.model_param) and [model.tune.param_range](#func.model.tune.param_range) functions described above in the *Common Helper Functions /* [Regularization Functions] section of *this Appendix*.
:::
:::
...
Data frame representing the *UMG Effect Model* trained (and also *regularized* if the [lambda](#func.train_user_movie_genre_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
The *MSE* value calculated for the *UMG Effect Model* with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
The *RMSE* value calculated for the *UMG Effect Model* with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
::: {.noteblock data-latex=""}
In the code snippet above, the function `calc_date_general_effect` is called internally from the [train_UMGY_effect](#func.train_UMGY_effect) function described below in this section.
:::
:::
...
This is a helper function that calculates the _Date General Effect_ (optionally *regularizes* if the [lambda](#func.calc_date_general_effect.cv.args.lambda) argument value is non-default) of the user ratings using the _K-Fold Cross Validation_ and is intended to be used in other custom functions (such as [train_UMGY_effect](#func.train_UMGY_effect)) described below.
...
::: {.noteblock data-latex=""}
In the above code snippet, the `calc_date_general_effect.cv` function is called internally from the [train_UMGY_effect.cv](#func.train_UMGY_effect.cv) function, along with the [calc_UMGY_effect](#func.calc_UMGY_effect) function (both described below).
:::
:::
...
Under the hood, this function internally calls the [calc_date_general_effect](#func.calc_date_general_effect) function described above for each iteration of the *K-Fold Cross-Validation* applied to the [`edx_CV` Object] described below in the [Appendix B: Models Training Datasets]. It also calls internally the [union_cv_results](#func.union_cv_results) function described above in the *Data Helper Functions / [Models Training Functions]* section of this *Appendix* to aggregate the cross-validation results.
...
Data frame representing (*regularized* if [lambda](#func.calc_date_general_effect.cv.args.lambda) argument value is non-default) *Date General Effect*.
:::
...
::: {.noteblock data-latex=""}
In the code snippet above, the function `calc_UMGY_effect` is called internally from the [train_UMGY_effect](#func.train_UMGY_effect) function described below.
:::
:::
...
::: { #func.calc_UMGY_effect.args.date_general_effect .sidebar }
  - **date_general_effect:** Data frame representing the *Date General Effect*; 
...
This is a helper function that is intended to be used internally in the following functions (both described below in this section):
...
An example of using it in the [train_UMGY_effect](#func.train_UMGY_effect) function is shown above in the [Usage](#func.calc_UMGY_effect.usage) section of *This Description*:
...
Data frame representing the *UMGYE Model*.
:::
...
Under the hood, this function internally calls the [calc_date_general_effect](#func.calc_date_general_effect) and [calc_UMGY_effect](#func.calc_UMGY_effect) functions (both described above in this section).
...
Data frame representing the trained (and also *regularized* if [lambda](#func.train_UMGY_effect.args.lambda) argument value is non-default) *UMGY Effect Model*.
:::
...
::: {.noteblock data-latex=""}
This helper function is also used internally in the [regularize.test_lambda.UMGY_effect.cv](#func.regularize.test_lambda.UMGY_effect.cv) function (described below in section [UMGYE Model: Regularization] of *this Appendix*) as shown in the [Usage](#func.train_UMGY_effect.cv.usage) subsection of *This Function's Description Section* below.
:::
...
::: {.noteblock data-latex=""}
In the above code snippet, the `train_UMGY_effect.cv` function is called internally from the [regularize.test_lambda.UMGY_effect.cv](#func.regularize.test_lambda.UMGY_effect.cv) function, along with the [calc_UMGY_effect_RMSE.cv](#func.calc_UMGY_effect_RMSE.cv) function (both described below in [UMGYE Model: Regularization] section and this section, respectively).
:::
:::
...
Under the hood, this function internally calls the [calc_date_general_effect.cv](#func.calc_date_general_effect.cv) and [calc_UMGY_effect](#func.calc_UMGY_effect) functions (both described above in this section).
...
Data frame representing the *UMGY Effect Model* trained (and also *regularized* if the [lambda](#func.train_UMGY_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
Validates the *UMGY Effect (UMGYE) Model* produced by the  [train_UMGY_effect](#func.train_UMGY_effect) or [train_UMGY_effect.cv](#func.train_UMGY_effect.cv) function call.

::: {.noteblock data-latex=""}
This helper function is used internally in the [calc_UMGY_effect_MSE.cv](#func.calc_UMGY_effect_MSE.cv) function (described below in this section) as shown in the [Usage](#func.calc_UMGY_effect_MSE.usage) subsection of *This Function's Description Section* below.
:::
...
::: {.noteblock data-latex=""}
In the code snippet above, the function `calc_UMGY_effect_MSE` is called internally from the [calc_UMGY_effect_MSE.cv](#func.calc_UMGY_effect_MSE.cv) function described below in this section.
:::
:::
...
::: { #func.calc_UMGY_effect_MSE.args.UMGY_effect .sidebar }
  - **UMGY_effect:** Data frame representing the *UMGY Effect Model*. 
...
Validates the *UMGY Effect (UMGYE) Model* (produced by the [train_UMGY_effect](#func.train_UMGY_effect) or [train_UMGY_effect.cv](#func.train_UMGY_effect.cv) function call) with use of *K-Fold Cross-Validation* as described in [Section 29.6 *Cross validation / K-fold cross validation*](https://rafalab.dfci.harvard.edu/dsbook-part-2/ml/resampling-methods.html#k-fold-cross-validation) of the _Course Textbook (New Edition)_[@IDS2_29-6_KFCV].

::: {.noteblock data-latex=""}
This helper function is used internally in the [calc_UMGY_effect_RMSE.cv](#func.calc_UMGY_effect_RMSE.cv) function (described below in this section) as shown in the [Usage](#func.calc_UMGY_effect_MSE.cv.usage) subsection of *This Function's Description Section* below.
:::
...
::: {.noteblock data-latex=""}
In the code snippet above, the function `calc_UMGY_effect_MSE.cv` is called internally from the [calc_UMGY_effect_RMSE.cv](#func.calc_UMGY_effect_RMSE.cv) function described below in this section.
:::
:::
...
::: {.noteblock data-latex=""}
This function internally calls the [calc_UMGY_effect_MSE](#func.calc_UMGY_effect_MSE) function for each iteration of the *K-Fold Cross-Validation*, calculating the *MSE* for the corresponding validation dataset of the [`edx_CV` Object], and then calculates the mean of the obtained results.
...
The *MSE* value calculated for the *UMGY Effect Model* with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
::: {.noteblock data-latex=""}
This helper function is also used internally in the [regularize.test_lambda.UMGY_effect.cv](#func.regularize.test_lambda.UMGY_effect.cv) function described below in section [UMGYE Model: Regularization] of *this Appendix*.
:::
...
::: {.noteblock data-latex=""}
This function internally calls the [calc_UMGY_effect_MSE.cv](#func.calc_UMGY_effect_MSE.cv) function described above, to calculate the *MSE* for the *UMGYE Model* with use of *K-Fold Cross-Validation*, and then calculates the square root of the result.
...
The *RMSE* value calculated for the *UMGY Effect Model* with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
Internally it uses the [train_UMGY_effect.cv](#func.train_UMGY_effect.cv) and [calc_UMGY_effect_RMSE.cv](#func.calc_UMGY_effect_RMSE.cv) functions described above to validate the *UMGYE Model* for the given $\lambda$ with use of the *K-Fold Cross-Validation* method, as described in [Section 29.6 *Cross validation / K-fold cross validation*](https://rafalab.dfci.harvard.edu/dsbook-part-2/ml/resampling-methods.html#k-fold-cross-validation) of the _Course Textbook (New Edition)_[@IDS2_29-6_KFCV], where the value of $K$ is determined by the length of the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets] (in *this Project* we use $K = 5$).
...
::: {.noteblock data-latex=""}
In the code snippet above, the function `calc_day_general_effect` is called internally along with the [calc_UMGY_SmoothedDay_effect](#func.calc_UMGY_SmoothedDay_effect) function from the [train_UMGY_SmoothedDay_effect](#func.train_UMGY_SmoothedDay_effect) function (both described below in this section).
:::
:::
...
This is a helper function that is intended for use in other helper functions such as [calc_day_general_effect.cv](#func.calc_day_general_effect.cv) and [train_UMGY_SmoothedDay_effect](#func.train_UMGY_SmoothedDay_effect) (both described below in this section). 
...
Data frame representing (*regularized* if [lambda](#func.calc_day_general_effect.args.lambda) argument value is non-default) *DG Effect*.
:::
...
This is a helper function that calculates the *Day General Effect* (optionally *regularizes* if the [lambda](#func.calc_day_general_effect.cv.args.lambda) argument value is non-default) of the user ratings using the K-Fold Cross Validation_ and is intended to be used in other custom functions (such as [train_UMGY_effect](#func.train_UMGY_effect)) described below.
...
::: {.noteblock data-latex=""}
In the above code snippet, the `calc_day_general_effect.cv` function is called internally from the [train_UMGY_effect.cv](#func.train_UMGY_effect.cv) function, along with the [calc_UMGY_effect](#func.calc_UMGY_effect) function (both described below).
:::
:::
...
Under the hood, this function internally calls the [calc_day_general_effect](#func.calc_day_general_effect) function described above for each iteration of the *K-Fold Cross-Validation* applied to the [`edx_CV` Object] described below in the [Appendix B: Models Training Datasets]. It also calls internally the [union_cv_results](#func.union_cv_results) function described above in the *Data Helper Functions / [Models Training Functions]* section of this *Appendix* to aggregate the cross-validation results.
...
Data frame representing (*regularized* if [lambda](#func.calc_day_general_effect.cv.args.lambda) argument value is non-default) *Day General Effect*.
:::
...
Using local fitting, fits a polynomial surface determined by the *Day General Effect* (hereafter *DG Effect* or *DGE* for short) information passed in the [de_bias.dat](#func.loess_de.args.de_bias.dat) argument.
...
::: {.noteblock data-latex=""}
In the code snippet above, the `loess_de` is called internally from the [calc_UMGY_SmoothedDay_effect](#func.calc_UMGY_SmoothedDay_effect) function described below.
:::
:::
...
Dataframe object representing the *Day General Effect* data. 
...
(Integer, $NA$ by default) The degree of the polynomials to be used (passed in the same-named argument to the internally called [stats::loess](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/loess) function).

::: {.noteblock data-latex=""}
When the value is $NA$ (default), $1$ is passed in the same-named argument to the internally called [stats::loess](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/loess) function. 
:::
:::
...
(Numeric, $NA$ by default ) The parameter $\alpha$, which controls the degree of smoothing (passed in the same-named argument to the internally called [stats::loess](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/loess) function).

::: {.noteblock data-latex=""}
When the value is $NA$ (default), $0.75$ is passed in the same-named argument to the internally called [stats::loess](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/loess) function. 
:::
:::
:::
...
This function is a wrapper for the [stats::loess](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/loess) `R` function, which is called internally to make a required fit.

::: {.noteblock data-latex=""}
This is a helper function intended for use in other helper functions, such as the [calc_UMGY_SmoothedDay_effect](#func.calc_UMGY_SmoothedDay_effect) described below in this section.
:::
:::
...
Object of class `loess` returned by the internally called function [stats::loess](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/loess).
:::
...
The source code of the [loess_de](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L71) function is shown below:
```{r eval=FALSE}
...
Builds the *User+Movie+Genre+Year Effect (UMGYDE) Model* based on the *Day General Effect* data structure returned by the [calc_day_general_effect](#func.calc_day_general_effect) or [calc_day_general_effect.cv](#func.calc_day_general_effect.cv) function (both described above) and passed in the [date_general_effect](#func.calc_UMGY_SmoothedDay_effect.args.date_general_effect) argument.
...
::: {.noteblock data-latex=""}
In the code snippet above, the function `calc_UMGY_SmoothedDay_effect` is called internally from the [train_UMGY_effect](#func.train_UMGY_effect) function described below.
:::
:::
...
::: { #func.calc_UMGY_SmoothedDay_effect.args.date_general_effect .sidebar }
  - **date_general_effect:** Data frame representing the *Day General Effect*; 
...
An example of using it in the [train_UMGY_effect](#func.train_UMGY_effect) function is shown above in the [Usage](#func.calc_UMGY_SmoothedDay_effect.usage) section of *This Description*:
...
Data frame representing the *UMGYDE Model*.
:::
...
The source code of the [calc_UMGY_SmoothedDay_effect](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L76) function is shown below:
```{r eval=FALSE}
```

::: {.noteblock data-latex=""}
The source code of the [calc_UMGY_SmoothedDay_effect](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L76) function is also available in the [Utility Functions](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L3) section of the [UMGYD-effect.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L76) script on _GitHub_.
:::
:::
...
Under the hood, this function internally calls the [calc_day_general_effect](#func.calc_day_general_effect) and [calc_UMGY_effect](#func.calc_UMGY_effect) functions (both described above in this section).
...
Data frame representing the trained (and also *regularized* if [lambda](#func.train_UMGY_SmoothedDay_effect.args.lambda) argument value is non-default) *UMGY Effect Model*.
:::
...
Trains (and optionally *regularizes* if the [lambda](#func.train_UMGY_SmoothedDay_effect.cv.args.lambda) argument value is non-default) the *User+Movie+Genre+Year Effect (UMGYDE) Model* with use of *K-Fold Cross-Validation* as described in [Section 29.6 *Cross validation / K-fold cross validation*](https://rafalab.dfci.harvard.edu/dsbook-part-2/ml/resampling-methods.html#k-fold-cross-validation) of the _Course Textbook (New Edition)_[@IDS2_29-6_KFCV].

::: {.noteblock data-latex=""}
This helper function is also used internally in the [regularize.test_lambda.UMGYD_effect.cv](#func.regularize.test_lambda.UMGYD_effect.cv) function (described below in section [UMGYDE Model: Regularization] of *this Appendix*) as shown in the [Usage](#func.train_UMGY_SmoothedDay_effect.cv.usage) subsection of *This Function's Description Section* below.
:::
...
::: {.noteblock data-latex=""}
In the above code snippet, the `train_UMGY_SmoothedDay_effect.cv` function is called internally from the [regularize.test_lambda.UMGYD_effect.cv](#func.regularize.test_lambda.UMGYD_effect.cv) function, along with the [calc_UMGY_effect_RMSE.cv](#func.calc_UMGY_effect_RMSE.cv) function (both described below in [UMGYDE Model: Regularization] section and this section, respectively).
:::
:::
...
Under the hood, this function internally calls the [calc_day_general_effect.cv](#func.calc_day_general_effect.cv) and [calc_UMGY_effect](#func.calc_UMGY_effect) functions (both described above in this section).
...
Data frame representing the *UMGY Effect Model* trained (and also *regularized* if the [lambda](#func.train_UMGY_SmoothedDay_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
```{r eval=FALSE}
train_UMGY_SmoothedDay_effect.cv <- function(lambda = 0){
  if (is.na(lambda)) {
    stop("Function: train_UMGY_SmoothedDay_effect.cv
`lambda` is `NA`")
  }
  
  calc_day_general_effect.cv(lambda) |> calc_UMGY_effect()  
}
```
...
Validates the *UMGY Effect (UMGYDE) Model* produced by the  [train_UMGY_effect](#func.train_UMGY_effect) or [train_UMGY_effect.cv](#func.train_UMGY_effect.cv) function call.

::: {.noteblock data-latex=""}
This helper function is used internally in the [UMGY_SmoothedDay_effect.predict.cv](#func.UMGY_SmoothedDay_effect.predict.cv) function (described below in this section) as shown in the [Usage](#func.UMGY_SmoothedDay_effect.predict.usage) subsection of *This Function's Description Section* below.
:::
...
::: {.noteblock data-latex=""}
In the code snippet above, the function `UMGY_SmoothedDay_effect.predict` is called internally from the [UMGY_SmoothedDay_effect.predict.cv](#func.UMGY_SmoothedDay_effect.predict.cv) function described below in this section.
:::
:::
...
:::
::: { #func.UMGY_SmoothedDay_effect.predict.args.UMGYD_effect .sidebar }
  - **UMGYD_effect:** Data frame representing the *UMGY Effect Model*. 
...
The function calculates *Mean Squared Error (MSE)* for the *UMGYDE Model* passed in the [UMGYD_effect](#func.UMGY_SmoothedDay_effect.predict.args.UMGYD_effect) argument using the validation dataset passed in the [test_set](#func.UMGY_SmoothedDay_effect.predict.args.test_set) arg. 
:::
...
::: {.noteblock data-latex=""}
This helper function is used internally in the [calc_UMGY_SmoothedDay_effect.MSE.cv](#func.calc_UMGY_SmoothedDay_effect.MSE.cv) function (described below in this section) as shown in the [Usage](#func.calc_UMGY_SmoothedDay_effect.MSE.usage) subsection of *This Function's Description Section* below.
:::
...
:::
::: { #func.calc_UMGY_SmoothedDay_effect.MSE.args.UMGYD_effect .sidebar }
  - **UMGYD_effect:** Data frame representing the *UMGY Effect Model*. 
...
The function calculates *Mean Squared Error (MSE)* for the *UMGYDE Model* passed in the [UMGYD_effect](#func.calc_UMGY_SmoothedDay_effect.MSE.args.UMGYD_effect) argument using the validation dataset passed in the [test_set](#func.calc_UMGY_SmoothedDay_effect.MSE.args.test_set) arg. 
:::
...
Validates the *UMGY Effect (UMGYDE) Model* (produced by the [train_UMGY_effect](#func.train_UMGY_effect) or [train_UMGY_effect.cv](#func.train_UMGY_effect.cv) function call) with use of *K-Fold Cross-Validation* as described in [Section 29.6 *Cross validation / K-fold cross validation*](https://rafalab.dfci.harvard.edu/dsbook-part-2/ml/resampling-methods.html#k-fold-cross-validation) of the _Course Textbook (New Edition)_[@IDS2_29-6_KFCV].

::: {.noteblock data-latex=""}
This helper function is used internally in the [calc_UMGY_effect_RMSE.cv](#func.calc_UMGY_effect_RMSE.cv) function (described below in this section) as shown in the [Usage](#func.calc_UMGY_SmoothedDay_effect.MSE.cv.usage) subsection of *This Function's Description Section* below.
:::
...
::: {.noteblock data-latex=""}
In the code snippet above, the function `calc_UMGY_SmoothedDay_effect.MSE.cv` is called internally from the [calc_UMGY_effect_RMSE.cv](#func.calc_UMGY_effect_RMSE.cv) function described below in this section.
:::
:::
...
::: { #func.calc_UMGY_SmoothedDay_effect.MSE.cv.args.UMGYD_effect .sidebar }
  - **UMGYD_effect:** Data frame representing the *UMGY Effect Model*. 
...
The function calculates *MSE* for the *UMGYDE Model* passed in the [UMGYD_effect](#func.calc_UMGY_SmoothedDay_effect.MSE.cv.args.UMGYD_effect) argument using the validation datasets contained in the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets].

::: {.noteblock data-latex=""}
This function internally calls the [calc_UMGY_SmoothedDay_effect.MSE](#func.calc_UMGY_SmoothedDay_effect.MSE) function for each iteration of the *K-Fold Cross-Validation*, calculating the *MSE* for the corresponding validation dataset of the [`edx_CV` Object], and then calculates the mean of the obtained results.
...
The simplified version of the source code of the [calc_UMGY_SmoothedDay_effect.MSE.cv](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L147) function is shown below:
```{r eval=FALSE}
```
...
::: {.noteblock data-latex=""}
This helper function is also used internally in the [regularize.test_lambda.UMGYD_effect.cv](#func.regularize.test_lambda.UMGYD_effect.cv) function described below in section [UMGYDE Model: Regularization] of *this Appendix*.
:::
...
::: { #func.calc_UMGY_SmoothedDay_effect.RMSE.cv.args.UMGYD_effect .sidebar }
  - **UMGYD_effect:** Data frame representing the *UMGY Effect Model*. 
...
The function calculates *Root Squared Error (RMSE)* for the *UMGYDE Model* passed in the [UMGYD_effect](#func.calc_UMGY_SmoothedDay_effect.RMSE.cv.args.UMGYD_effect) argument using the validation datasets contained in the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets].

::: {.noteblock data-latex=""}
This function internally calls the [calc_UMGY_effect_MSE.cv](#func.calc_UMGY_effect_MSE.cv) function described above, to calculate the *MSE* for the *UMGYDE Model* with use of *K-Fold Cross-Validation*, and then calculates the square root of the result.
...
#### [regularize.train_UMGYD_effect](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L99) Function {#func.regularize.train_UMGYD_effect}
\

Trains (and optionally *regularizes* if the [lambda](#func.regularize.train_UMGYD_effect.args.lambda) argument value is non-default) the *User+Movie+Genre+Year Effect (UMGYDE) Model* using the training sample of the `Movielens` dataset passed in the [train_set](#func.regularize.train_UMGYD_effect.args.train_set) argument.
...
Data frame representing the trained (and also *regularized* if [lambda](#func.regularize.train_UMGYD_effect.args.lambda) argument value is non-default) *UMGY Effect Model*.
:::
...
The source code of the [regularize.train_UMGYD_effect](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L99) function is shown below:
```{r eval=FALSE}
```

::: {.noteblock data-latex=""}
The source code of the [regularize.train_UMGYD_effect](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L99) function is also available in the [Utility Functions](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L3) section of the [UMGYD-effect.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L99) script on _GitHub_.
:::
:::

\newpage

#### [regularize.train_UMGYD_effect.cv](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L86) Function {#func.regularize.train_UMGYD_effect.cv}
\
...
::: {.noteblock data-latex=""}
In the above code snippet, the `regularize.train_UMGYD_effect.cv` function is called internally from the [regularize.test_lambda.UMGYD_effect.cv](#func.regularize.test_lambda.UMGYD_effect.cv) function described below, along with the [calc_UMGY_SmoothedDay_effect.RMSE.cv](#func.calc_UMGY_SmoothedDay_effect.RMSE.cv) function described above in this section.
:::
:::
...
This is a helper function that is used internally in the [regularize.test_lambda.UMGYD_effect.cv](#func.regularize.test_lambda.UMGYD_effect.cv) function described below in this section as shown in the [Usage](#func.regularize.train_UMGYD_effect.cv.usage) section of *this Description* above.

Under the hood, it calls internally the [calc_day_general_effect.cv](#func.calc_day_general_effect.cv) and [calc_UMGY_effect](#func.calc_UMGY_effect) functions (both described above in this section).

::: {.noteblock data-latex=""}
For the *regularization technique* (described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-2]) to be correctly applied to the model being trained, the *regularization parameter* $\lambda$ passed in the [lambda](#func.regularize.train_UMGYD_effect.cv.args.lambda) argument must be greater than zero (when the $\lambda$ is $0$ the *regularization* is disabled).
:::
:::
...
Data frame representing the *UMGY Effect Model* trained (and also *regularized* if the [lambda](#func.regularize.train_UMGYD_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (in *this Project* we use $K = 5$).
:::
...
The source code of the [regularize.train_UMGYD_effect.cv](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L86) function is shown below:
```{r eval=FALSE}
```

::: {.noteblock data-latex=""}
The source code of the [regularize.train_UMGYD_effect.cv](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L86) function is also available in the [Regularization](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L224) section of the [UMGYD-effect.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L86) script on _GitHub_.
:::
:::

\newpage

#### [regularize.test_lambda.UMGYD_effect.cv](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L130) Function {#func.regularize.test_lambda.UMGYD_effect.cv}
\

Validates the *regularization parameter* $\lambda$ passed in the [lambda](#func.regularize.test_lambda.UMGYD_effect.cv.args.lambda) argument for the *User+Movie+Genre+Year Effect (UMGYDE) Model* in the *regularization process* using *penalized approach* with use of *K-Fold Cross-Validation*.
...
Internally it uses the [train_UMGY_effect.cv](#func.train_UMGY_effect.cv) and [calc_UMGY_effect_RMSE.cv](#func.calc_UMGY_effect_RMSE.cv) functions described above to validate the *UMGYDE Model* for the given $\lambda$ with use of the *K-Fold Cross-Validation* method, as described in [Section 29.6 *Cross validation / K-fold cross validation*](https://rafalab.dfci.harvard.edu/dsbook-part-2/ml/resampling-methods.html#k-fold-cross-validation) of the _Course Textbook (New Edition)_[@IDS2_29-6_KFCV], where the value of $K$ is determined by the length of the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets] (in *this Project* we use $K = 5$).
...
The source code of the [regularize.test_lambda.UMGYD_effect.cv](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L130) function is shown below:
```{r eval=FALSE}
```

::: {.noteblock data-latex=""}
The source code of the [regularize.test_lambda.UMGYD_effect.cv](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L130) function is also available in the [Regularization](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L224) section of the [UMGYD-effect.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L130) script on _GitHub_.
:::
:::
