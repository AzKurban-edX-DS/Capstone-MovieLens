...
Calculates the average value of the *Mean Squared Errors* calculated using the _K-Fold Cross-Validation_ method as described in [Section 30.6 *Cross validation*](https://rafalab.dfci.harvard.edu/dsbook-part-2/ml/resampling-methods.html#cross-validation) of the _Course Textbook (New Edition)_[@IDS2_30-6].
...
##### Details
\
This function internally uses the [base::union](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/sets) `R` function to combine the data from the list of data frames (result of the _N-Fold Cross-Validation_) passed in the [data_list] argument into a single data frame for further processing.
:::
...
Calculates the Mean Squared Errors (MSEs) for the *Overall Mean Rating (OMR) Model* using the *K-Fold Cross-Validation* method as described in section [OMR Model Building] of *this Report*.
...
The Mean Squared Errors (MSEs) values obtained from applying the *K-Fold Cross-Validation* method to the *Overall Average Rating (OMR) Model*.
:::
...
\
Calculates the Root Mean Squared Error (RMSE) for the *Overall Mean Rating (OMR) Model* using the *K-Fold Cross-Validation* method as described in section [OMR Model Building] of *this Report*.
...
This function internally calls the [naive_model_MSEs](#func.naive_model_MSEs) function described above.
:::
...
The Root Mean Squared Errors (RMSE) value obtained from applying the *K-Fold Cross-Validation* method to the *Overall Average Rating (OMR) Model*.
:::
...
Computes the *(Regularized) User+Movie (UM) Effect* using the training dataset passed in the [train_set](#func.train_user_movie_effect.args.train_set) parameter.
...
Trains the *(Regularized) User+Movie (UM) Effect Model* using the the _N-Fold Cross-Validation_ 
as described in [Section 30.6 *Cross validation / K-fold cross validation*](https://rafalab.dfci.harvard.edu/dsbook-part-2/ml/resampling-methods.html#k-fold-cross-validation) of the _Course Textbook (New Edition)_[@IDS2_30-6].
...
Data frame representing the *UM Effect Model* trained (and also *regularized* if the [lambda](#func.train_user_movie_effect.cv.args.lambda) argument value is other than the default) using _N-Fold Cross-Validation_.
:::
...
Calculates *Mean Squared Error (MSE)* for the *UM Effect (UME) Model* produced by the  [train_user_movie_effect](#func.train_user_movie_effect) or [train_user_movie_effect.cv](#func.train_user_movie_effect.cv) function call.
...
To calculate the *MSE* for the *UME Model* passed in the [um_effect](#func.calc_user_movie_effect_MSE.args.um_effect) argument, the function uses the validation dataset passed in the [test_set](#func.calc_user_movie_effect_MSE.args.test_set) arg. 
...
Calculates *Mean Squared Error (MSE)* for the *UM Effect (UME) Model* (produced by the [train_user_movie_effect](#func.train_user_movie_effect) or [train_user_movie_effect.cv](#func.train_user_movie_effect.cv) function call) using *N-Fold Cross-Validation*.
...
The function utilizes *N-Fold Cross-Validation* to calculate *MSE* for the *UME Model* passed in the [um_effect](#func.calc_user_movie_effect_MSE.cv.args.um_effect) argument using the validation datasets contained by the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets].

::: {.noteblock data-latex=""}
This function internally calls the [calc_user_movie_effect_MSE](#func.calc_user_movie_effect_MSE) function for each iteration of the *N-Fold Cross-Validation* method, calculating the *MSE* for the corresponding validation dataset of the [`edx_CV` Object], and then calculates the mean of the obtained results.
...
The *MSE* value calculated for the *UM Effect Model* using the *N-Fold Cross-Validation*, where the `N` is the length of the [`edx_CV` Object].
:::
...
Calculates *Root Mean Squared Error (RMSE)* for the *UM Effect (UME) Model* (produced by the [train_user_movie_effect](#func.train_user_movie_effect) or [train_user_movie_effect.cv](#func.train_user_movie_effect.cv) function call) using *N-Fold Cross-Validation*.
...
The function utilizes *N-Fold Cross-Validation* to calculate *RMSE* for the *UME Model* passed in the [um_effect](#func.calc_user_movie_effect_RMSE.cv.args.um_effect) argument using the validation datasets contained by the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets].

::: {.noteblock data-latex=""}
This function internally calls the [calc_user_movie_effect_MSE.cv](#func.calc_user_movie_effect_MSE.cv) function described above, to calculate the *MSE* for the *UME Model* using *N-Fold Cross-Validation*, and then calculates the square root of the result.
...
The *RMSE* value calculated for the *UM Effect Model* using the *N-Fold Cross-Validation*, where the `N` is the length of the [`edx_CV` Object].
:::
