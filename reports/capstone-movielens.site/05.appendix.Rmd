...
Returns an object of class `POSIXct` (see {DateTimeClasses}(https://www.rdocumentation.org/link/DateTimeClasses?package=base&version=3.6.2)). 
...
Prints the system date and time to the currently opened log (using the [open_logfile](#func.open_logfile) function described above) at the time the function is called and returns the system's idea of the current date and time.
...
It first prints the value returned by the `date` function to the currently opened log, then returns the object returned by the `Sys.time` function.
:::
...
It first prints the value returned by the `date` function, then returns an object of class `difftime` representing the time elapsed since the date and time specified by the [start](#func.print_end_date.args.start) argument.
:::
...
Returns an object of class `difftime` representing the time elapsed since the date and time specified by the [start](#func.print_end_date.args.start) argument. 
...
Returns an object of class `difftime` representing the time elapsed since the date and time specified by the [start](#func.put_end_date.args.start) argument. 
...
#### [clamp](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L6) Function {#func.clamp}
\
...
The source code of the [clamp](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L6) function is shown below:
```{r eval=FALSE}
