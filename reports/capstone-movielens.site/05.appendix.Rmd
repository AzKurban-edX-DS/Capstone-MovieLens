...
```
  ...
  2. Checks if the *step size* of the generated sequence is less than the minimum allowed value (in the current implementation, we use `1e-13`), and if so, exits the loop. Otherwise, proceeds to the [next step](#func.model.tune.param_range.details.step3):  
```{r eval=FALSE}
...
::: {.noteblock data-latex=""}
In the code snippet above, we use the custom function [put_log1](#func.put_log2) described above in the [Logging Functions] section of *This Appendix*.
:::
:::
...
::: { #func.model.tune.param_range.details.step6 .sidebar }
  6. Calls the helper function [tune.model_param](#func.tune.model_param), passing the vector created earlier in [step 3](#func.model.tune.param_range.details.step3) to the [param_values](#func.tune.model_param.args.param_values) argument (along with the values of the rest arguments: [fn_tune.test.param_value](#func.tune.model_param.args.fn_tune.test.param_value), [break.if_min](#func.tune.model_param.args.break.if_min), [steps.beyond_min](#func.tune.model_param.args.steps.beyond_min)) of the called function and stores the returned result in the local variable `tuned_result`:
```{r eval=FALSE}
...
::: { #func.model.tune.param_range.details.step7 .sidebar }
  7. Extracts the internal data frame stored in the variable `tuned_result$tuned.result` from the data returned by the [tune.model_param](#func.tune.model_param) helper function (and saved in the local variable `tuned_result`) in the [previous step](#func.model.tune.param_range.details.step6) and stores it in the local variable `tuned.result`. Then stores `tuned_result` along with the necessary auxiliary information in the cache file with the full name created earlier in [step 4](#func.model.tune.param_range.details.step4):
```{r eval=FALSE}
      tuned.result <- tuned_result$tuned.result
...
::: { #func.model.tune.param_range.details.step .sidebar }
