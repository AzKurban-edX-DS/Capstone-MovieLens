...
::: { #func.mean_reg.args.lambda .sidebar }
  - **lambda:** (Optional, 0 by default) _Regularization parameter_ $\lambda$. 

:::
::: { #func.mean_reg.args.na.rm .sidebar }
  - **na.rm:** (Optional, `TRUE` by default) Logical value indicating whether `NA` values should be stripped before the computation proceeds.
...
::: { #func.train_user_movie_effect.cv.args.lambda .sidebar }
  - **lambda:** (Optional, $0$ by default) Regularization parameter $\lambda$. 
...
::: {.noteblock data-latex=""}
This is an auxiliary function intended to be called internally from the [calc_user_movie_effect_MSE.cv](#func.calc_user_movie_effect_MSE.cv) as shown in the [Usage](#func.calc_user_movie_effect_MSE.usage) section above of *this Description*.
:::
:::
...
```{r main-script@l571u, eval=FALSE}
  lambdas <- seq(0, 1, 0.1)
  cv.UME.preset.result <- 
    tune.model_param(lambdas, regularize.test_lambda.UM_effect.cv)
```
...
::: {.noteblock data-latex=""}
In the [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L571) above this function is passed in the [fn_tune.test.param_value](#func.tune.model_param.args.fn_tune.test.param_value) argument to the [tune.model_param](#func.tune.model_param) function described above in the section [Model Tuning Utils] of *this Appendix*.
