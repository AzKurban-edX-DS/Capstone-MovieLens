...
We call this function internally from the functions that compute the _penalized estimates_  as described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the *Course Textbook (New Edition)*[@IDS2_24-2].
...
The `K` value for the *K-Fold Cross-Validation* method is determined by the length of the [`edx_CV` Object] described below in the [Appendix B: Models Training Datasets] (in our project we use `K = 5`).
...
The Mean Squared Error (MSE) values obtained from applying the *K-Fold Cross-Validation* method to the *Overall Average Rating (OMR) Model*, where the `K` value is determined by the length of the [`edx_CV` Object] (in our project we use `K = 5`).
...
The Root Mean Squared Errors (RMSE) value obtained from applying the *K-Fold Cross-Validation* method to the *Overall Average Rating (OMR) Model*, where the `K` value is determined by the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
The regularization is committed through the *penalized regression* by computing the *penalized mean* performed by the [mean_reg](#func.mean_reg) function described above in the *Common Helper Functions /* [Regularization Functions] section of *this Appendix*.
:::
...
The `K` value for the *K-Fold Cross-Validation* method is determined by the length of the [`edx_CV` Object] (in our project we use `K = 5`).

::: {.noteblock data-latex=""}
The *Regularization technique* (described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-2]) is applied to the model being trained only when the *regularization parameter* $\lambda$ passed in the [lambda](#func.train_user_movie_effect.cv.args.lambda) argument, has non-default value (to be more exact, greater than zero).
:::
:::
...
Data frame representing the *UM Effect Model* trained (and also *regularized* if the [lambda](#func.train_user_movie_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
The `K` value for the *K-Fold Cross-Validation* is determined by the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
:::
...
The *MSE* value calculated for the *UM Effect Model* with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
The *RMSE* value calculated for the *UM Effect Model* with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
Data frame representing the *UMG Effect Model* trained (and also *regularized* if the [lambda](#func.train_user_movie_genre_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
The *MSE* value calculated for the *UMG Effect Model* with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
The *RMSE* value calculated for the *UMG Effect Model* with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
The regularization is committed through the *penalized regression* by computing the *penalized mean* performed by the internally called [mean_reg](#func.mean_reg) function described above in the *Common Helper Functions /* [Regularization Functions] section of *this Appendix*.
:::
...
::: {.noteblock data-latex=""}
The *Regularization technique* (described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-2]) is applied to the model being trained only when the *regularization parameter* $\lambda$ passed in the [lambda](#func.train_UMGY_effect.args.lambda) argument, has non-default value (to be more exact, greater than zero).
:::
:::
...
::: {.noteblock data-latex=""}
The *Regularization technique* (described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-2]) is applied to the model being trained only when the *regularization parameter* $\lambda$ passed in the [lambda](#func.train_UMGY_effect.cv.args.lambda) argument, has non-default value (to be more exact, greater than zero).
:::
:::
...
Data frame representing the *UMGY Effect Model* trained (and also *regularized* if the [lambda](#func.train_UMGY_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
The *MSE* value calculated for the *UMGY Effect Model* with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
The *RMSE* value calculated for the *UMGY Effect Model* with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
This is a helper function that computes the *Day General Effect(hereafter DG Effect or DGI for short)* (optionally *regularizes* if the [lambda](#func.calc_day_general_effect.args.lambda) argument value is non-default) of user ratings on a *given day since the earliest record* in the `edx` dataset, using a *training dataset* passed in the [train_set](#func.calc_day_general_effect.args.train_set) argument, and is intended for use in other helper functions such as [calc_day_general_effect.cv](#func.calc_day_general_effect.cv) and [train_UMGY_SmoothedDay_effect](#func.train_UMGY_SmoothedDay_effect) (both described below in this section).
...
The *Regularization technique* (described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-2]) is applied to the model being trained only when the *regularization parameter* $\lambda$ passed in the [lambda](#func.calc_day_general_effect.args.lambda) argument, has non-default value (to be more exact, greater than zero).
The regularization is committed through the *penalized regression* by computing the *penalized mean* performed by the internally called [mean_reg](#func.mean_reg) function described above in the *Common Helper Functions /* [Regularization Functions] section of *this Appendix*.
:::
...
::: {.noteblock data-latex=""}
The *Regularization technique* (described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-2]) is applied to the model being trained only when the *regularization parameter* $\lambda$ passed in the [lambda](#func.calc_day_general_effect.cv.args.lambda) argument, has non-default value (to be more exact, greater than zero).
The regularization is committed through the *penalized regression* by computing the *penalized mean* performed by the internally called [mean_reg](#func.mean_reg) function described above in the *Common Helper Functions /* [Regularization Functions] section of *this Appendix*.
:::
:::
...
Builds the *User+Movie+Genre+Year Effect (UMGYDE) Model* based on the *Day General Effect* data structure returned by the [calc_date_general_effect](#func.calc_date_general_effect) or [calc_date_general_effect.cv](#func.calc_date_general_effect.cv) function (both described above) and passed in the [date_general_effect](#func.calc_UMGY_SmoothedDay_effect.args.date_general_effect) argument.
...
```{r eval=FALSE}
calc_UMGY_SmoothedDay_effect <- function(date_general_effect){
  date_general_effect |>
    group_by(year) |>
    summarise(ye = mean(de, na.rm = TRUE))
}
...
```{r eval=FALSE}
  rglr.UMGYD_effect <- edx |> train_UMGY_SmoothedDay_effect(UMGYDE.rglr.best_lambda)
```
:::
...
::: {.noteblock data-latex=""}
The *Regularization technique* (described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-2]) is applied to the model being trained only when the *regularization parameter* $\lambda$ passed in the [lambda](#func.train_UMGY_SmoothedDay_effect.args.lambda) argument, has non-default value (to be more exact, greater than zero).
:::
:::
...
```{r eval=FALSE}
train_UMGY_SmoothedDay_effect <- function(train_set, lambda = 0){
  if (is.na(lambda)) {
...
```{r eval=FALSE}
  train_UMGY_SmoothedDay_effect.cv(lambda) |>
    calc_UMGY_effect_RMSE.cv()
```
...
::: {.noteblock data-latex=""}
The *Regularization technique* (described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-2]) is applied to the model being trained only when the *regularization parameter* $\lambda$ passed in the [lambda](#func.train_UMGY_SmoothedDay_effect.cv.args.lambda) argument, has non-default value (to be more exact, greater than zero).
:::
:::
...
Data frame representing the *UMGY Effect Model* trained (and also *regularized* if the [lambda](#func.train_UMGY_SmoothedDay_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
...
```{r eval=FALSE}
    cv_fold_dat$validation_set |> UMGY_SmoothedDay_effect.predict(UMGYD_effect)
```
...
```{r eval=FALSE}
UMGY_SmoothedDay_effect.predict <- function(test_set, UMGYD_effect){
  test_set |>
    left_join(edx.user_effect, by = "userId") |>
    left_join(rglr.UM_effect, by = "movieId") |>
    left_join(rglr.UMG_effect, by = "movieId") |>
...
```{r eval=FALSE}
    cv_fold_dat$validation_set |> calc_UMGY_SmoothedDay_effect.MSE(UMGYD_effect)
```
...
```{r eval=FALSE}
calc_UMGY_SmoothedDay_effect.MSE <- function(test_set, UMGYD_effect){
  test_set |>
    left_join(edx.user_effect, by = "userId") |>
    left_join(rglr.UM_effect, by = "movieId") |>
    left_join(rglr.UMG_effect, by = "movieId") |>
...
```{r eval=FALSE}
  UMGY_effect_RMSE <- sqrt(calc_UMGY_SmoothedDay_effect.MSE.cv(UMGYD_effect))
```
...
```{r eval=FALSE}
calc_UMGY_SmoothedDay_effect.MSE.cv <- function(UMGYD_effect){
  start <- put_start_date()
  UMGY_effect_MSEs <- sapply(edx_CV, function(cv_fold_dat){
    cv_fold_dat$validation_set |> calc_UMGY_SmoothedDay_effect.MSE(UMGYD_effect)
  })
...
```{r eval=FALSE}
cv.UMGYD_effect.RMSE <- calc_UMGY_SmoothedDay_effect.RMSE.cv(cv.UMGYD_effect)
```
:::
...
```{r eval=FALSE}
calc_UMGY_SmoothedDay_effect.RMSE.cv <- function(UMGYD_effect){
  UMGY_effect_RMSE <- sqrt(calc_UMGY_effect_MSE.cv(UMGYD_effect))
  put_log2("%1-Fold Cross Validation ultimate RMSE: %2", 
           CVFolds_N, 
           UMGY_effect_RMSE)
...
::: {.noteblock data-latex=""}
The complete source code of the functions described in this section is available in the  [Regularization](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L224) section of the [UMGYD-effect.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/UMGYD-effect.functions.R#L3) script on _GitHub_.
:::
...
Trains (and optionally *regularizes* if the [lambda](#func.regularize.train_UMGYD_effect.cv.args.lambda) argument value is non-default) the *User+Movie+Genre+Year Effect (UMGYDE) Model* with use of *K-Fold Cross-Validation* as described in [Section 29.6 *Cross validation / K-fold cross validation*](https://rafalab.dfci.harvard.edu/dsbook-part-2/ml/resampling-methods.html#k-fold-cross-validation) of the _Course Textbook (New Edition)_[@IDS2_29-6_KFCV].

::: {.noteblock data-latex=""}
This helper function is also used internally in the [regularize.test_lambda.UMGYD_effect.cv](#func.regularize.test_lambda.UMGYD_effect.cv) function (described below in section [UMGYDE Model: Regularization] of *This Appendix*) as shown in the [Usage](#func.regularize.train_UMGYD_effect.cv.usage) subsection of *This Function's Description Section* below.
:::
...
::: {.noteblock data-latex=""}
The *Regularization technique* (described in [Section 24.2 *Penalized least squares*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#penalized-least-squares) of the _Course Textbook (New Edition)_[@IDS2_24-2]) is applied to the model being trained only when the *regularization parameter* $\lambda$ passed in the [lambda](#func.regularize.train_UMGYD_effect.cv.args.lambda) argument, has non-default value (to be more exact, greater than zero).
:::
:::
...
Data frame representing the *UMGY Effect Model* trained (and also *regularized* if the [lambda](#func.regularize.train_UMGYD_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (in our project we use `K = 5`).
:::
