...
##### Details
\
This function internally uses the [base::union](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/sets) `R` function to combine the data from the list of data frames (result of the *K-Fold Cross-Validation*) passed in the [data_list] argument into a single data frame for further processing.
:::
...
The Mean Squared Errors (MSEs) values obtained from applying the *K-Fold Cross-Validation* method to the *Overall Average Rating (OMR) Model*, where the `K` value is determined by the length of the [`edx_CV` Object].
...
The function calculates *RMSE* for the *UME Model* passed in the [um_effect](#func.calc_user_movie_effect_RMSE.cv.args.um_effect) argument using the validation datasets contained in the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets].
...
The *RMSE* value calculated for the *UM Effect Model* using the *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object].
:::
