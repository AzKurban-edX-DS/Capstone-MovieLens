...
::: { #func.tune.model_param.args.param_values .sidebar }
  - **param_values:** Vector containing a sorted list of $\lambda$ values to search for the best one during the model regularization process; 
...
::: { #func.tune.model_param.args.break.if_min .sidebar }
  - **break.if_min:** (`TRUE` by default) Boolean value that determines whether the algorithm should terminate after completing the number of steps specified by the [steps.beyond_min](#func.tune.model_param.args.steps.beyond_min) argument from the moment the minimum `RMSE` value is found;

:::

::: { #func.tune.model_param.args.steps.beyond_min .sidebar }
  - **steps.beyond_min:** (default 2; only takes effect if [break.if_min](func.tune.model_param.args.break.if_min) is `TRUE`) Specifies the number of steps after finding the minimum *RMSE* value after which the algorithm should terminate.
...
##### Details
\
The algorithm iterates through the $\lambda$ parameter values from the list passed in the [param_values](#func.tune.model_param.args.param_values) argument, computing the model's *RMSE* for each of them, using the helper function specified by the [fn_tune.test.param_value](#fn_tune.test.param_value) argument.

When the minimum *RMSE* value is reached, the algorithm terminates if [break.if_min](#func.tune.model_param.args.break.if_min) is `FALSE`.
Otherwise, the algorithm continues iterating and terminates after the number of cycles specified by the [steps.beyond_min](#func.tune.model_param.args.steps.beyond_min) argument.
...
::: {.noteblock data-latex=""}
The source code of the [tune.model_param](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L126) function is also available in the [Regularization](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L69) section of the [common-helper.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L126) script on _GitHub_.
:::
