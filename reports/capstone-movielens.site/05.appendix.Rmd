...
Trains (and optionally *regularizes* if the [lambda](#) argument is non-default) the *User+Movie Effect (UME) Model* using the training dataset passed in the [train_set](#func.train_user_movie_effect.args.train_set) argument.
...
Trains (and optionally *regularizes* if the [lambda](#) argument is non-default) the *User+Movie Effect (UME) Model* using the training dataset passed in the [train_set](#func.train_user_movie_genre_effect.args.train_set) argument.
...
::: { #func.train_user_movie_genre_effect.args.train_set .sidebar }
  - **train_set:** Training dataset; 
...
Data frame representing the trained (and also *regularized* if [lambda](#func.train_user_movie_genre_effect.args.lambda) arg value other than default) *UM Effect Model*.
:::
...
Data frame representing the *UM Effect Model* trained (and also *regularized* if the [lambda](#func.train_user_movie_genre_effect.cv.args.lambda) argument value is other than the default) with use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object]..
:::
...
The function calculates *Mean Squared Error (MSE)* for the *UME Model* passed in the [um_effect](#func.calc_user_movie_genre_effect_MSE.args.um_effect) argument using the validation dataset passed in the [test_set](#func.calc_user_movie_genre_effect_MSE.args.test_set) arg. 
...
The function calculates *MSE* for the *UME Model* passed in the [um_effect](#func.calc_user_movie_genre_effect_MSE.cv.args.um_effect) argument using the validation datasets contained in the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets].
...
The function calculates *Root Squared Error (RMSE)* for the *UME Model* passed in the [um_effect](#func.calc_user_movie_genre_effect_RMSE.cv.args.um_effect) argument using the validation datasets contained in the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets].
...
Validates the *regularization parameter* $\lambda$ passed in the [lambda](#func.regularize.test_lambda.UMG_effect.cv.args.lambda) argument for the *User+Movie Effect (UME) Model* in the *regularization process* using *penalized approach* with use of *K-Fold Cross-Validation*.
...
The *Root Mean Squared Errors (RMSE)* value computed for the UME Model applying the *regularization parameter* $\lambda$ passed in the [lambda](#func.regularize.test_lambda.UMG_effect.cv.args.lambda) argument.
