...
##### Value
\
Vector containing the index of the element corresponding to the minimum `RMSE` value in a given sequence, along with the indices of its nearest neighboring elements:
```{r eval=FALSE}
...
8. Determines the endpoints for a new interval of $\lambda$ values in the neighborhood of the minimum *RMSE* value figured out so far, with use of the helper function [get_fine_tune.param.endpoints.idx](#func.get_fine_tune.param.endpoints.idx) described above, to improve the accuracy of the *RMSE* calculation in the next iteration, and stores them in the `seq_start` and `seq_end` local variables:
```{r eval=FALSE}
