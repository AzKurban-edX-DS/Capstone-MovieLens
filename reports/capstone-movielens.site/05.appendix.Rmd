...
```{r logging-functions@l39u, eval=FALSE}
  print(get_log1(msg_template, arg1))
```

::: {.noteblock data-latex=""}
In the [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/logging-functions.R#L39) above, the function `get_log1` is called internally from the [print_log1](#func.print_log1) function described below in *this Section*.
:::
...
::: {.noteblock data-latex=""}
This is an auxiliary function intended for use in other custom logging functions, such as the [print_log1](#func.print_log1) (as shown in the [Usage](#func.get_log1.usage) section of *this Description*) and [put_log1](#func.put_log1) described below in *this Section*.
:::
...
```{r main-script@l177u, eval=FALSE}
which_min_deviation <- deviation[which.min(deviation.RMSE)]
min_rmse = min(deviation.RMSE)
...
#### [msg.set_arg](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functionsR#L453) Function {#func.msg.set_arg}
\
...
The source code of the [msg.set_arg](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functionsR#L453) function is shown below:
```{r eval=FALSE}
...
::: {.noteblock data-latex=""}
The source code of the [msg.set_arg](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functionsR#L453) function is also available in the [Utility Functions](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L449) section of the [common-helper.functions.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functionsR#L453) script on _GitHub_.
:::
:::
...
```{r main-script@l719u, eval=FALSE}
...
```{r main-script@l347u, eval=FALSE}
...
::: {.noteblock data-latex=""}
The $K$ value for the *K-Fold Cross-Validation* method is determined by the length of the [`edx_CV` Object] described below in [Appendix B: Models Training Datasets] (in *this Project* we use $K = 5$).
:::
...
::: { #func.RMSEs.AddRow.args.before .sidebar }
  - ***before:*** (Optional, `NULL` by default) One-based row index where to add the new rows, if NULL (default): after last row;
...
```{r common-helper.func@l66u, eval=FALSE}
  RMSEs |>
...
::: {.noteblock data-latex=""}
In the [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L66) above, the function `RSME.tibble.col_width` is called internally from the [RMSE_kable](#func.RMSE_kable) function described below in *this Section*.
:::
:::
...
::: {.noteblock data-latex=""}
As a utility, this function is intended for use in other user-defined functions, such as the [RMSE_kable](#func.RMSE_kable) (as shown above in the [Usage](#func.get_log1.usage) section of *this Description*) and [RMSE.Total_kable](#func.RMSE.Total_kable) described below in *this Section*.
:::
:::
...
```{r main-script@l205u, eval=FALSE}
...
```{r main-script@l2390u, eval=FALSE}
...
::: {#func.get_fine_tune.param.endpoints.idx.args.preset.result .sidebar }
  - **preset.result:** Data frame that has an *RMSE* column containing the `RMSE` values corresponding to the *regularization parameter* $\lambda$ values for the model being regularized, as described in section [Details](#func.tune.model_param.details) section of the [tune.model_param](#func.tune.model_param) function description below.
...
::: { #func.model.tune.param_range.details.step2 .sidebar }
  2. Checks if the *step size* of the generated sequence is less than the minimum allowed value (in the current implementation, we use `1e-13`), and if so, exits the loop and goes to the [Finalizing Execution](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L426) section of the code described in section [Finalizing Execution](#func.model.tune.param_range.details.finalizing) subsection below. 
...
::: {.noteblock data-latex=""}
In the code snippet above, we use the custom function [put_log2](#func.put_log2) described above in section [Logging Functions] of *this Appendix*.
:::  

::: { #func.model.tune.param_range.details.step3 .sidebar }
  3. If the *step size* is no less than the minimum allowed value (`seq_increment >= 1e-13` in our case), creates a vector containing a sequence of parameter values (as described in section [loop_starter](#func.model.tune.param_range.args.loop_starter) argument description above) using the following [line of code](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/support-functions/common-helper.functions.R#L256):
```{r eval=FALSE}
    test_param_vals <- seq(seq_start, seq_end, seq_increment)
```
...
::: {.noteblock data-latex=""}
In the code snippet above, we use the custom function [put_log1](#func.put_log1) described above in section [Logging Functions] of *this Appendix*.
:::
:::
...
```{r main-script@l1838u, eval=FALSE}
...
::: { #func.data.plot.args.xlabel .sidebar }
  - **xlabel:** (Character string, `NULL` by default) The $x$ axis label. If `NULL`, the value of the `xname` parameter is used for the label; 
...
::: {.noteblock data-latex=""}
In the code snippet above this function is passed in the [fn_tune.test.param_value](#func.tune.model_param.args.fn_tune.test.param_value) argument to the [tune.model_param](#func.tune.model_param) function described above in the section [Model Tuning Utils] of *this Appendix*.
...
::: {.noteblock data-latex=""}
In the code snippet above the `train_UMGY_SmoothedDay_effect.RMSE.cv.degree0` function is passed in the [fn_tune.test.param_value](#func.tune.model_param.args.fn_tune.test.param_value) argument to the [tune.model_param](#func.tune.model_param) function described above in the section [Model Tuning Utils] of *this Appendix*.
:::
:::
...
::: {.noteblock data-latex=""}
In the code snippet above the `train_UMGY_SmoothedDay_effect.RMSE.cv.degree1` function is passed in the [fn_tune.test.param_value](#func.model.tune.param_range.args.fn_tune.test.param_value) argument to the [model.tune.param_range](#func.model.tune.param_range) function described above in the section [Model Tuning Utils] of *this Appendix*.
:::
:::
...
::: {.noteblock data-latex=""}
In the code snippet above the `train_UMGY_SmoothedDay_effect.RMSE.cv.degree2` function is passed in the [fn_tune.test.param_value](#func.model.tune.param_range.args.fn_tune.test.param_value) argument to the [model.tune.param_range](#func.model.tune.param_range) function described above in the section [Model Tuning Utils] of *this Appendix*.
:::
:::
...
::: {.noteblock data-latex=""}
In the code snippet above this function is passed in the [fn_tune.test.param_value](#func.model.tune.param_range.args.fn_tune.test.param_value) argument to the [model.tune.param_range](#func.model.tune.param_range) function described above in the section [Model Tuning Utils] of *this Appendix*.
