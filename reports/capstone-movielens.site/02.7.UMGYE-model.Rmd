...
Now, we can refine our _User+Movie+Genre+Year Effect Model_ by retraining on the entire `edx` dataset with the best value of the $\lambda$ parameter we just figured out (let's call it *Regularized User+Movie+Genre+Year Effect Model* or *Regularized UMGYE Model* for short), for the definitive _RMSE_ calculation and use in subsequent models:
...
::: {.noteblock data-latex=""}
In the code snippet above we use the [train_UMGY_effect](#func.train_UMGY_effect) function described in the *Models Training: Support Functions /* [UMGYE Model: Utility Functions] section of [Appendix A: Support Functions].
:::
...
Finally, we calculate the *RMSE* for the ultimately *Regularized UMGYE Model* and add the definitive result to our _Result Table_:
```{r, eval=FALSE}
