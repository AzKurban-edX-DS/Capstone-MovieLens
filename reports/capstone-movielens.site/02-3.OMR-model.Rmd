...
The following custom functions described in [Appendix A: Support Functions] of *this Report*, are used in the code snippets above:

  - [naive_model_MSEs](#func.naive_model_MSEs) (described in section [Overall Mean Rating (OMR) Model Helper Functions]) to compute _Mean Squared Errors_ using _5-Fold Cross Validation_ method;
...
::: {.noteblock data-latex=""}
In the code snippet above, we use the custom function [naive_model_RMSE](#func.naive_model_RMSE) described in the [Overall Mean Rating (OMR) Model Helper Functions] section of [Appendix A: Support Functions] to calculate *RMSEs* for the sequence of the deviation values.
:::
