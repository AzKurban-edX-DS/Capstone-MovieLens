...
We know that the estimate that minimizes the RMSE is the least squares estimate of $\mu$ and, in this case, is the average of all ratings. We calculate this using the folowing [line of code](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L106):
```{{r main-script-l106, echo=TRUE, }
mu <- mean(edx$rating)
mu
```

If we predict all unknown ratings with $\hat{\mu}$, we can obtain *RMSE* using the following [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L108): 
```{r main-script-l08 }
...
The following custom functions described in [Appendix A: Support Functions], are used in the code snippets above:
  ...
  - [naive_model_MSEs](#func.naive_model_MSEs) (described in section [OMR Model: Helper Functions]) to compute *Mean Squared Errors* (hereafter *MSE* for short) with the use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (described in detail in [Appendix B: Models Training Datasets]) to which the *K-Fold Cross-Validation* is applied (in *this Project* we use $K = 5$);

  - [data.plot](#func.data.plot) (described in section [Data Visualization Functions]) for the *MSE* data visualization.
...
::: {.noteblock data-latex=""}
In the code snippet above, we use the function [naive_model_RMSE](#func.naive_model_RMSE) described in the [OMR Model: Helper Functions] section of [Appendix A](#appndx_a) to calculate *RMSEs* for the sequence of the deviation values.
:::

Let's quickly analyze the resulting `deviation.RMSE` vector we have just got. The following [data visualization](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L189) code will help us with this:
```{r main-script-l189}
...
::: {.noteblock data-latex=""}
To print the log, we use the function [print_log1](#func.print_log1) described in the [Logging Functions] section of [Appendix A](#appndx_a).
:::
