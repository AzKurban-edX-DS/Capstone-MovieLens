...
Now, we are ready to compute the *Mean Squared Errors (MSEs)* for the samples used in _K-Fold Cross-Validation_ (additionally using the [clamp](#func.clamp) helper function described in the [Common Helper Functions]/[Utility Functions] section of [Appendix A](#appndx_a)).
...
::: {.noteblock data-latex=""}
In the code snippet above, we also use the user-defined function [mse](#func.mse) described in the [Common Helper Functions]/[(Root) Mean Squared Error Calculation] section of [Appendix A](#appndx_a).
:::
