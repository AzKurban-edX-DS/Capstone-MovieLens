...
### User Effect Analysis {#sec.user_effect_analysis}
\

To improve our model let's now take into consideration user effects as explained in [Section 24.1 *Case study: recommendation systems / User effects*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#user-effects) of the *Course Textbook (New Edition)*[@IDS2_24-1_UE].

Let's start our analysis by calculating the average rating for each user using the following [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L231):
```{r main-script@l231, eval=FALSE}
...
We also use the user-defined helper function [put_log](#func.put_log) described in section [Logging Functions] of [Appendix A](#appndx_a).
:::  

The [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L263) below visualizes the frequency of the users' average ratings. As the author of the *Course Textbook* shows, such a visualization allows us to see that there is substantial variability in the average ratings across users[@IDS2_24-1_UE]: 
```{r  main-script@l263}
...
Following the further explanation provided in [Section 24.1](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#user-effects) of the *Course Textbook* (mentioned above in the [previous Section](#sec.user_effect_analysis)), to account for this variability, we use a linear model with a _treatment effect_  $\alpha_i$ for each user. The sum $\mu+\alpha_i$ can be interpreted as the typical rating user $i$ gives to movies. So we write the model as follows:
$$
Y_{i,j} = \mu + \alpha_i + \varepsilon_{i,j}
$$

Statistics textbooks refer to the $\alpha$s as treatment effects. In the Netflix challenge papers, they refer to them as _bias_[@IDS2_24-1_UE; @MFT_RS].
...
As it is stated in [Section 33.7.6: *User effects*](https://rafalab.dfci.harvard.edu/dsbook/large-datasets.html#user-effects) of the *Course Textbook (First Edition)*, it can be shown that the least squares estimate $\hat{\alpha}_i$ is just the average of $y_{i,j} - \hat{\mu}$ for each user $i$. So we can compute them this way[@IDS_E1_33-7-6]:
```{r eval=FALSE}
a <- rowMeans(y - mu, na.rm = TRUE)
```

These considerations allow us to compute a _User Mean Ratings_ using the following [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L231):
...
The following [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L336) plots a histogram that provides a visual representation of the variability of the *User Effect* across users in the *data frame object* we have just obtained:
```{r main-script@l336}
par(cex = 0.7)
...
Now, we are ready to compute the *Mean Squared Errors (MSEs)* for the samples used in _K-Fold Cross-Validation_ (additionally using the [clamp](#func.clamp) helper function described in section [Utility Functions] of [Appendix A](#appndx_a)).
...
::: {.noteblock data-latex=""}
In the code snippet above, we also use the user-defined function [mse](#func.mse) described in section [(Root) Mean Squared Error Calculation] of [Appendix A](#appndx_a).
:::
...
Finally, we add the *RMSE* value calculated for the *UE Model* to our *Result Table* and print the table using the following [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L372):
