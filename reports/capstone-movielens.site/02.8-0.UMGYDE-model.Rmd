...
If we define $d_{i,j}$ as the _number of days since the earliest record_ in the `edx` dataset for movie $j$ rated by user $i$, then the formula \@res(eq:UMGYE-model) describing the _UMGYE Model_, for the current model, takes the form:
...
Therefore, the formula \@res(eq:year-effect) for calculation the prediction of a _year effect_ as a residual, for a _smoothed day effect_ 
$$
...
We have already explained the idea of _Linear Model Regularization_ in the [UME Model Regularization] section above. We have also seen how the formula \@res(eq:ME-penalty) for adding a penalty to the _UME Model_ is transformed into the formula \@res(eq:GE-penalty) for the _UMGE Model_ and then into the formula \@res(eq:GYE-penalty) for the _UMGYE Model_. For the current model, this formula takes the form:
...
And the formula \@res(eq:ME-regularized) for calculating the values of the _treatment effect_ that minimizes the equation will take the form:
