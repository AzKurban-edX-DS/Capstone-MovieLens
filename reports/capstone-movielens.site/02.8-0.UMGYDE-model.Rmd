...
Since the `degree` parameter can only take three discrete values, 0, 1, and 2, we will perform the tuning in three stages - one for each possible value of the `degree`.

For each value of the `degree` parameter, we will divide each stage into the following two steps for tuning the model by the `span` parameter:
