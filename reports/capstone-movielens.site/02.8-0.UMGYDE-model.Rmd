...
We will start by calling the [train_UMGY_SmoothedDay_effect.cv](#func.train_UMGY_SmoothedDay_effect.cv) with default argument values, which, in addition to not use *regularization technique* yet (which will be used in further training later for the current model), means that the [loess](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/loess) function will be implicitly called with `degree = 1` and `span = 0.75` argument values by the another auxiliary function [loess_de](#func.loess_de) (described in section [UMGYDE Model: Utility Functions] of [Appendix A](#appndx_a)) included in the call chain that, actually, calls the [loess](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/loess).
