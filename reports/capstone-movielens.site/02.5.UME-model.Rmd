...
```{r}
print(UM_effect.loop_starter)
print(UME.rglr.fine_tune.cache.path)
print(UME.rglr.fine_tune.cache.base_name)
```
```{r, eval=FALSE}
...
Now, we can refine our _User+Movie Effect Model_ by retraining on the entire `edx` dataset with the best value of the $\lambda$ parameter we just figured out (let's call it *Regularized User+Movie Effect Model*), for the definitive _Root Mean Squared Error_ calculation and use in subsequent models.
```{r, eval=FALSE}
