...
Below, we provide the most significant part of the code for training our model with the use of *K-Fold Cross-Validation*, where the `K` is the length of the [`edx_CV` Object] (described in detail in [Appendix B: Models Training Datasets]) to which the *K-Fold Cross-Validation* is applied (in our project we use `K = 5`):
```{r eval=FALSE}
