...
As explained in the [Section 24.1, Subsection *Movie effects*](https://rafalab.dfci.harvard.edu/dsbook-part-2/highdim/regularization.html#sec-movie-effects) of the *Textbook* (mentioned earlier in section [Movies' Popularity]), for the *Movie effects* we can use a linear model with a _treatment effect_ $\beta_j$ for each movie, which can be interpreted as the movie effect, or the difference between the average rating for movie $j$ and the overall average $\mu$[@IDS2_24-1_ME]: 
$$
...
::: {.noteblock data-latex=""}
In the code snippet above we use the [train_user_movie_effect.cv](#func.train_user_movie_effect.cv) helper function described in section [UME Model: Utility Functions] of [Appendix A](#appndx_a).
:::
...
::: {.noteblock data-latex=""}
In the code snippet above, we use the [calc_user_movie_effect_RMSE.cv](#func.calc_user_movie_effect_RMSE.cv) function described in section [UME Model: Utility Functions] of [Appendix A](#appndx_a). 
:::
...
3. **Retraining:** (Described in subsection [UME Model Regularization: *Retraining on the `edx` with the best* $\lambda$]) Retraining the model on the entire `edx` dataset with the best value of $\lambda$ determined in the previous step.
...
This approach will have our desired effect: when our sample size $n_j$ is very large, we obtain a stable estimate and the penalty $\lambda$ is effectively ignored since $n_j+\lambda \approx n_j$. Yet when the $n_j$ is small, then the estimate $\hat{\beta}_i(\lambda)$ is shrunken towards 0. The larger the $\lambda$, the more we shrink[@IDS_E1_33-9-2].
...
For this purpose, we will use the user-defined helper function [tune.model_param](#func.tune.model_param), passing the [regularize.test_lambda.UM_effect.cv](#func.regularize.test_lambda.UM_effect.cv) (another custom helper function) as the value of its argument [fn_tune.test.param_value](#func.tune.model_param.args.fn_tune.test.param_value).

::: {.noteblock data-latex=""}
The functions [tune.model_param](#func.tune.model_param) and [regularize.test_lambda.UM_effect.cv](#func.regularize.test_lambda.UM_effect.cv) are described in the sections [Model Tuning Utils](#appndx_a.CHF.model_tuning_utils) and  [UME Model: Regularization], respectively, of [Appendix A: Support Functions].
:::
...
::: {.noteblock data-latex=""}
The helper function [get_fine_tune.param.endpoints](#func.get_fine_tune.param.endpoints) used in the code snippet above is described in the sections [Model Tuning Utils](#appndx_a.CHF.model_tuning_utils) of [Appendix A: Support Functions].
:::
...
#### UME Model Regularization: *Retraining on the `edx` with the best* $\lambda$
\
...
::: {.noteblock data-latex=""}
In the code snippet above we use the [train_user_movie_effect](#func.train_user_movie_effect) function described in section [UME Model: Utility Functions] of [Appendix A](#appndx_a).
:::
