...
::: {.noteblock data-latex=""}
The complete source code of the _User+Movie Effect_ computation described in this section is available in the [UME Model Building](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L456) section of the [capstone-movielens.main.R](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L456) script on _GitHub_.
:::

Below, we provide the most significant part of the code for training our model with the use of *K-Fold Cross-Validation*, where the $K$ is the length of the [`edx_CV` Object] (described in detail in [Appendix B: Models Training Datasets]) to which the *K-Fold Cross-Validation* is applied (in *this Project* we use $K = 5$):
```{r eval=FALSE}
  cv.UM_effect <- train_user_movie_effect.cv()
```

::: {.noteblock data-latex=""}
In the code snippet above we use the [train_user_movie_effect.cv](#func.train_user_movie_effect.cv) function described in the *Models Training: Support Functions /* [UME Model: Utility Functions] section of [Appendix A](#appndx_a).
:::
...
We can now construct predictors and see how much the `RMSE` improves[@IDS2_24-1_ME].

The following [line of code](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L513) allows us to perform this task:
```{r eval=FALSE}
...
Finally, we add the obtained result to our *Result Table* and print the table using the following [piece of code](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L):
```{r eval=FALSE}
...
In the code snippet above, we use the following functions described in [Appendix A: Support Functions]: 
  ...
  -  to calculate the *RMSE* for the *Current Model*;
...
We will implement the _Regularization_ method on our  models (starting from the current model) in following three steps:
  ...
  1. **Pre-configuration:** Preliminary determination of the optimal range of *regularization parameter* $\lambda$ values for the *K-Fold Cross-Validation* samples, where the $K$ is the length of the [`edx_CV` Object] (described in detail in [Appendix B: Models Training Datasets]) to which the *K-Fold Cross-Validation* is applied (in *this Project* we use $K = 5$);
...
The following [piece of code](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L571) performs this operation:
```{r, eval=FALSE}
lambdas <- seq(0, 1, 0.1)
...
The following [code snippet](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L608) prepares the interval of values for the $\lambda$ parameter, over which the operation has to be done:
```{r, eval=FALSE}
endpoints <- 
...
And the next [piece of code](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L617) below accomplishes the task of *fine-tuning* the model:
```{r, eval=FALSE}
...
#### UME Model Regularization: Retraining the Model with the best $\lambda$
\
...
The following [piece of code](https://github.com/AzKurban-edX-DS/Capstone-MovieLens/blob/main/r/src/capstone-movielens.main.R#L664) performs this operation:
```{r, eval=FALSE}
...
Finally, we calculate the *RMSE* for the ultimately *Regularized UME Model* and add the definitive result to our _Result Table_:
```{r, eval=FALSE}
...
In the code snippet above, we use the following functions described in [Appendix A: Support Functions]: 
  ...
  - [calc_user_movie_effect_RMSE.cv](#func.calc_user_movie_effect_RMSE.cv) (described in the *Models Training: Support Functions /* [UME Model: Utility Functions] section) to calculate the *RMSE* for the *UME Model*;
